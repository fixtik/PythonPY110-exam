...
def pk_inc(start_value: int = 1) -> int:  #инкримент
    pk = start_value
    while True:
        yield pk
        pk +=1
...
def get_title() -> str: #выдача названия из файла
    title_list = []
...
def get_number(wtw: str = 'year') -> int: #генерация случайного числа (год или кол-во страниц)
    if wtw == 'year':
...
def get_isbin() -> str: #генерация isbin
    fake = Faker()
    return fake.numerify(text='%%%-%-%%%%-%%%%-%%%-%%')


def get_float(wtw: str = 'rating') -> float: #получение рейтинга или цены
    if wtw == 'rating':
...
def get_author() -> list[str]: #получение списка авторов
...
def dit_creator(count: int = 5) -> list: #сборка словаря
    pk = pk_inc()
    title = get_title()
    result = []
